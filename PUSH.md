...
API 基地址：`http://<host>:3000`，文档：`/docs`。Web 界面内置（媒体库 / 文件管理 / 浏览 / 个人资料 / 设定 / 整理历史）。
API 令牌（`API_TOKEN`）首次启动若未设置会**随机生成**并打印到日志；手动设置须 **≥ 16 个字符**，否则会被忽略并随机重置。Web 界面走登录 JWT，不依赖它。管理员初始密码用 `SUPERUSER_PASSWORD` 固定（不设则随机生成于日志）。
