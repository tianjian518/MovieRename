...
except Exception:  # MovieRename 已移除媒体服务器
    MediaServerChain = None
try:
    from app.chain.search import SearchChain
except Exception:  # MovieRename 已移除搜索功能
    SearchChain = None
...
except Exception:  # MovieRename 已移除定时任务调度
    Scheduler = None
...
async def proxy_img(
    ...
    cookies = (
        ...
        if (use_cookies and MediaServerChain is not None)
        else None
    )
...
def run_scheduler(jobid: str, _: User = Depends(get_current_active_superuser)):
    ...
    if Scheduler is None:
        return schemas.Response(success=False, message="MovieRename 已移除定时任务调度")
