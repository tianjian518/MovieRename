...
except Exception:  # MovieRename 已移除 AI 智能体，相关能力自动降级为空操作
    ReplyMode = None
    prompt_manager = None
    agent_manager = None
...
except Exception:  # MovieRename 已移除订阅功能
    SubscribeChain = None
...
class FailedRetryScheduler:
    ...
    async def _flush_retry_transfer(self, group_key: str):
        ...
        # MovieRename 已移除 AI 智能体，智能体重试整理降级为空操作
        if agent_manager is None:
            logger.info("智能体重试整理未启用（AI 智能体已移除），跳过")
            return
